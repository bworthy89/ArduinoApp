...
// #define DISPLAY_DIN 16  // MOSI
// #define DISPLAY_CLK 15  // SCK
...
// Operating mode
bool testMode = false;        // When true, keyboard output is disabled
bool keyboardEnabled = true;  // Master keyboard enable
...
void setup() {
  ...
  // Initialize keyboard HID (Pro Micro only)
  Keyboard.begin();
...
void processCommand(String& input) {
  ...
  else if (strcmp(cmd, "KEYBOARD_MODE") == 0) {
    keyboardEnabled = jsonDoc["params"]["enabled"] | true;
    sendOk();
  }
...
void handleInputChanges() {
  if (testMode || !keyboardEnabled) return;

  // === Handle keyboard output based on input changes ===
  // (Generated code goes here)
  // Example:
  // if (inputStates[0] && !prevStates[0]) {
  //   Keyboard.press('a');
  //   Keyboard.release('a');
  // }
}
